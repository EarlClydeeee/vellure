  ...
  return (
    <section className="container mx-auto px-4 py-10 md:py-14">
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-5">
        <Link
          href={large.href}
          className="group relative flex min-h-[280px] cursor-pointer overflow-hidden rounded-2xl md:min-h-[360px] md:row-span-2"
        >
          <Image
            src={large.image}
            alt={large.title}
        ...
        <div className="flex flex-col gap-4 md:gap-5">
          {small.map((banner) => (
            <Link
              key={banner.id}
              href={banner.href}
              className="group relative flex min-h-[160px] cursor-pointer overflow-hidden rounded-2xl md:min-h-[170px]"
            >
              <Image
                src={banner.image}
                alt={banner.title}
              ...
              <div className="relative flex h-full flex-col justify-center p-5 text-white md:p-6">
                <p className="text-xs font-medium text-sky-200 md:text-sm">{banner.subtitle}</p>
                <h3 className="mt-0.5 text-lg font-bold md:text-xl">{banner.title}</h3>
